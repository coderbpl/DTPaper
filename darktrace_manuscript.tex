...
calibrated explainable scorer that statistically matches a black-box baseline while
adding interpretability (McNemar $p=2.8\times10^{-4}$), and achieves $100\%$ tamper
detection at $\sim$8.7k items/s with valid STIX~2.1 export. An integration ablation
...
Under pooled training the model is strong on well-represented languages (EN $0.908$,
$N{=}1757$; RU $0.916$, $N{=}114$); zero-shot transfer preserves English ($0.916$) but
shows the expected penalty on RU/DE/FR (Table~\ref{tab:r-ml}, Fig.~\ref{fig:transfer}). A
...
Over $500$ evidence items the local hash-chain backend sustains $8747.6$ items/s (p50
$0.094$~ms, p95 $0.233$~ms); the chain verified and the adversarial tamper test was
detected (Table~\ref{tab:r-int}). The extraction layer exported $300$ findings into a valid
STIX~2.1 bundle of $301$ objects ($300$ indicators) in $0.302$~s.
...
$p=2.79\times10^{-4}$) while overlapping it on ranking---i.e.\ \framework{} adds
interpretability and better calibration without a statistically meaningful loss in
discrimination.
...
\midrule
Severity & Explainable Ens. & 0.9290 [0.9187,\,0.9394]$^{\dagger}$ & --- & \multirow{2}{*}{McNemar $p=2.79\!\times\!10^{-4}$} \\
...
and \texttt{table\_stats.csv}. Experiment scripts persist per-instance predictions to
\texttt{darktrace\_results/preds/}; re-running the driver after a full experiment run adds
the paired-bootstrap macro-F1 difference for scoring, per-language macro-F1 bootstrap CIs,
and the Friedman$+$Nemenyi critical-difference diagram.
...
characterized transfer, a calibrated explainable scorer at parity with a black box,
